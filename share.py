...
class TicketGenerator:
    def __init__(self):
        # URLs das fontes do Google (Roboto) para garantir o visual
        ...
        # Carrega as fontes da nuvem
        self.font_title = self.get_font(self.FONT_BOLD_URL, 36)   # Cidade (Um pouco menor para caber melhor)
        self.font_label = self.get_font(self.FONT_REG_URL, 18)    # Labels
        self.font_value = self.get_font(self.FONT_BOLD_URL, 48)   # Valor Total
        self.font_small = self.get_font(self.FONT_REG_URL, 15)    # Detalhes
        self.font_footer = self.get_font(self.FONT_ITALIC_URL, 12) # Rodapé

    def get_font(self, url, size):
        """Baixa a fonte e retorna o objeto ImageFont"""
        try:
            resp = requests.get(url, timeout=5)
            resp.raise_for_status()
            return ImageFont.truetype(io.BytesIO(resp.content), size)
        except Exception as e:
            print(f"Erro ao baixar fonte: {e}. Usando padrão.")
    ...
    def create_ticket(self, destination, total_value, daily_value, days, vibe, currency):
        ...
        # --- DESTINO (Layout Inteligente) ---
        dest_text = destination.upper()
        wrapper = textwrap.TextWrapper(width=16) # Quebra com 16 caracteres
        dest_lines = wrapper.wrap(text=dest_text)

        # Calcula a altura de uma linha para espaçamento
        bbox = draw.textbbox((0, 0), "A", font=self.font_title)
        line_height = bbox[3] - bbox[1] + 10 

        # Ponto inicial Y para o bloco de texto do destino
        current_y = 160
        ...
        # Ajusta a posição para os próximos elementos
        current_y += 20 # Espaço extra após o nome da cidade
