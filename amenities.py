...
class AmenitiesGenerator:
    """
    Gera links profundos (Deep Links) para Google Maps, Events e Songkick
    baseado no Perfil (Vibe + Estilo) e nas Datas da viagem.
    """
    ...
    def generate_links(self, destination, vibe, style, start_date=None, end_date=None):
        """Retorna um dicionário com URLs prontas para uso"""
        ...
        # 1. FOOD (Google Maps)
        # Ex: "luxury fine dining paris top rated gastronomy"
        q_food = f"{style_mod} {destination} {keywords['food']}"
        url_food = f"https://www.google.com/maps/search/{self._clean_url(q_food)}"
        
        # 2. EVENTS (Google Search / Songkick Logic)
        # Se tiver data, refina a busca temporal
        date_str = ""
        if start_date:
            # Ex: "concerts Paris January 2026"
        ...
        # Uma busca específica para achar "jóias escondidas"
        ...
        return {
            ...
            "labels": {
                "food_label": f"Sabor {vibe.capitalize()}",
                "event_label": f"Agenda {date_str}" if date_str else "Agenda Local",
